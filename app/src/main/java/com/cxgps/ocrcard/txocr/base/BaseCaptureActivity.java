package com.cxgps.ocrcard.txocr.base;
...
import com.cxgps.ocrcard.txocr.bean.BankCardBean;
import com.cxgps.ocrcard.txocr.bean.BankInfo;
import com.cxgps.ocrcard.txocr.bean.DriverCardBean;
import com.cxgps.ocrcard.txocr.bean.DriverCardInfo;
import com.cxgps.ocrcard.txocr.bean.IDCardBackBean;
import com.cxgps.ocrcard.txocr.bean.IDCardFrontBean;
import com.cxgps.ocrcard.txocr.bean.IdCardInfo;
import com.cxgps.ocrcard.txocr.common.CardType;
import com.cxgps.ocrcard.txocr.common.YTServerAPI;
import com.cxgps.ocrcard.txocr.widget.LoadingDialog;
...
public abstract class BaseCaptureActivity extends BasePermissionFragmentActivity
                ...
                mHandler.post(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            }else if(cartType == CardType.TYPE_DRIVER_RUN || cartType == CardType.TYPE_DRIVER){

                                isSuccessful = handleDriverCardData(isSuccessful, bundle, responseBody);
                            } else {
     ...
     * @return
     */
    protected boolean handleDriverCardData(boolean isSuccessful, Bundle bundle, String responseBody) {

        DriverCardBean  driverCardBean = JSON.parseObject(responseBody, DriverCardBean.class);
        ...
        if (driverCardBean.errorcode == 0) {
            List<DriverCardBean.ItemsBean> items = driverCardBean.items;
            if (items != null && items.size() > 0) {
                for (DriverCardBean.ItemsBean item : items) {
                    if (getString(R.string.txt_card_number).equals(item.item)) {
                        String idCardNum = item.itemstring;
                        driverCardInfo.setDriverIdCard(idCardNum);
                        isSuccessful = true;
                    } else if (getString(R.string.txt_driver_name).equals(item.item)) {
                        driverCardInfo.setDriverName(item.itemstring);
                        isSuccessful = true;
                    }else if (getString(R.string.txt_driver_national).equals(item.item)) {
                        driverCardInfo.setDriverNational(item.itemstring);
                    } else if (getString(R.string.txt_driver_address).equals(item.item)) {
                        driverCardInfo.setDriverAddress(item.itemstring);
                    } else if (getString(R.string.txt_driver_birth).equals(item.item)) {
                        driverCardInfo.setDriverBirth(item.itemstring);
                    } else if (getString(R.string.txt_driver_signdate).equals(item.item)) {
                        driverCardInfo.setDriverSignDate(item.itemstring);
                    }else if (getString(R.string.txt_driver_type).equals(item.item)) {
                        driverCardInfo.setDriverType(item.itemstring);
                    }else if (getString(R.string.txt_driver_validate).equals(item.item)) {
                        driverCardInfo.setDriverValiDate(item.itemstring);
                    }else if (getString(R.string.txt_driver_startdate).equals(item.item)) {
                        driverCardInfo.setDriverStartDate(item.itemstring);
                    }
                }
            }
            image  = tempFileSate;
            if (!TextUtils.isEmpty(path)) {
                mServer.savaBitmap(image, path);
                driverCardInfo.setImageUrl(path);
            }

            bundle.putSerializable(BUNDLE_DATA, driverCardInfo);
        }
        return isSuccessful;
    }
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            try {
                                ...
                                String fileData = new String(Base64.encode(bytes,Base64.DEFAULT));
                                if (cartType == CardType.TYPE_BANK) {
                                    mServer.bankCardOcr(fileData);
                                } else if(cartType == CardType.TYPE_DRIVER || cartType == CardType.TYPE_DRIVER_RUN){
                                    tempFileSate = fileData;
                                    mServer.driverLicenseOcr(fileData, cartType == CardType.TYPE_DRIVER_RUN ? 0 : 1);
